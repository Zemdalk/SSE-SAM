...
def get_img_num_per_cls(cls_num, total_num, cls_indices, imb_type, imb_factor):
    # This function is excerpted and adapted from a publicly available code [commit 6feb304, MIT License]:
    # https://github.com/kaidic/LDAM-DRW/blob/master/imbalance_cifar.py
    img_max = total_num / cls_num
    img_num_per_cls = []
    if imb_type == 'exp':
        for cls_idx in range(cls_num):
            num = img_max / (imb_factor**(cls_idx / (cls_num - 1.0))) if cls_idx in cls_indices else 0
